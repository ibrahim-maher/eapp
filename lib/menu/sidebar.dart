...
import '../models/index.dart'
    show AppModel, BackDropArguments, Category, CategoryModel, UserModel,FilterAttributeModel;
...
class MenuBarState extends State<SideBarMenu> {
  ...
  Widget drawerItem(
    ...
    switch (value) {
      ...
      case 'Age':
        {
          final TextStyle textStyle = Theme.of(context).textTheme.bodyMedium  ?? TextStyle();

          // Get the icon color from the theme
  ...
  Selector<FilterAttributeModel, List<SubAttribute>> buildListAge({
    required TextStyle textStyle,
    required Color iconColor,
    required Color textColor,
  }) {
    return Selector<FilterAttributeModel, List<SubAttribute>>(
      selector: (_, provider) => provider.lstProductAttribute
          ?.where((attr) => attr.slug == 'pa_age')
          .expand((attr) => provider.lstSubAttribute[attr.id] ?? [])
          .cast<SubAttribute>()  // Ensure the correct type
          .toList() ?? [],
      builder: (context, subAttributes, child) {
        return ExpansionTile(
          initiallyExpanded: true,
          expandedCrossAxisAlignment: CrossAxisAlignment.start,
          tilePadding: const EdgeInsets.only(left: 16, right: 8),
          title: Text(
            S.of(context).byAge.toUpperCase() , // Display "By Brand" text
            style: textStyle.copyWith(
              fontSize: 14,
              fontWeight: FontWeight.w600,
            ),
          ...
          iconColor: iconColor,
          collapsedIconColor: textColor,
          children: _buildSubAttributeTiles(context, subAttributes, textStyle),
        );
      },
    );
  }
  ...
  Selector<FilterAttributeModel, List<SubAttribute>> buildListBrand({
    required TextStyle textStyle,
    required Color iconColor,
    required Color textColor,
  }) {
    return Selector<FilterAttributeModel, List<SubAttribute>>(
      ...
      builder: (context, subAttributes, child) {
        return ExpansionTile(
          ...
          tilePadding: const EdgeInsets.only(left: 16, right: 8),
          title: Text(
            S.of(context).byBrand.toUpperCase(),  // Display "By Brand" text
            style: textStyle.copyWith(
              fontSize: 14,
              fontWeight: FontWeight.w600,
            ),
  ...
  List<Widget> _buildSubAttributeTiles(
    ...
    for (var i = 0; i < subAttributes.length; i++) {
      ...
      tiles.add(
        Container(
          color:i.isOdd
              ? null
              : Theme.of(context).colorScheme.secondary.withOpacity(0.1),// Alternating background color
          child: ListTile(
            title: Padding(
              padding: const EdgeInsets.only(left: 20),
              child: Text(
                (subAttribute.name ?? '').toUpperCase(),  // Convert name to uppercase
                style: textStyle.copyWith(fontSize: 14),
              ),
            ),
            trailing: (subAttribute.count != null && subAttribute.count! > 0)
                ? Text(
              S.of(context).nItems(subAttribute.count!),
              style: textStyle.copyWith(fontSize: 12),
            )
                : null,
            onTap: () => _navigateToSubAttributeProducts(context, subAttribute),
          ),
        ),
      );
    }
  ...
  void _navigateToSubAttributeProducts(BuildContext context, SubAttribute subAttribute) {
    final filterAttributeModel = Provider.of<FilterAttributeModel>(context, listen: false);

    // Find the parent attribute for the sub-attribute
    final parentAttribute = filterAttributeModel.lstProductAttribute?.firstWhere(
          (attr) => attr.id == filterAttributeModel.lstSubAttribute.entries
          .firstWhere((entry) => entry.value.contains(subAttribute))
          .key,
      orElse: () => FilterAttribute.fromJson({}),
    );

    // Construct the terms for the attribute and sub-attribute
    final terms = <String>[];
    if (subAttribute.id != null) {
      terms.add('${subAttribute.id}');
    }
    final attributeTerm = terms.join(',');

    // Build the config as a JSON object with "advancedParams"
    final Map<String, dynamic> config = {
      ...
      'is_all_data': 'true',
      'advancedParams': {
        'attribute': parentAttribute?.slug ?? '',  // Dynamic attribute value
        'attribute_term': attributeTerm,           // Dynamic attribute_term value
      }
    };

    // Navigate to the backdrop screen with the updated config and arguments
    FluxNavigate.pushNamed(
      RouteList.backdrop,
      arguments: BackDropArguments(
        cateId: null,
        cateName: null,
        config: config,  // Passing the JSON config
        data: [],        // Modify the data if needed
        showCountdown: false,
    ...
    print('Navigated to products with sub-attribute: ${subAttribute.name} (${parentAttribute?.name})');
  }
